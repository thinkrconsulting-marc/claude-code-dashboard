          ...
          <motion.div key={cmd?.cmd} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.02 }}>
            <Card className="hover:bg-accent/20 transition-colors" style={{ boxShadow: 'var(--shadow-sm)' }}>
              <CardContent className="p-3 flex items-center gap-3">
                ...
                <code className="font-mono text-sm font-medium text-primary min-w-[180px]">{cmd?.cmd ?? ''}</code>
                <span className="text-sm text-muted-foreground flex-1">{cmd?.desc ?? ''}</span>
                <Badge variant="secondary" className="text-[10px] flex-shrink-0">{cmd?.cat ?? ''}</Badge>
